...
void IGrill::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
  switch (event) {
...
uint16_t IGrill::get_handle_(const char *service, const char *characteristic) {
  auto *chr = this->parent()->get_characteristic(esp32_ble_tracker::ESPBTUUID::from_raw(service),
                                                 esp32_ble_tracker::ESPBTUUID::from_raw(characteristic));
  if (chr == nullptr) {
...
void IGrill::read_temperature_unit_(uint8_t *raw_value, uint16_t value_len) {
  ...
  ESP_LOGI(TAG, "Device reports temperature unit: %s (this unit can be changed in iGrill app)",
           this->unit_of_measurement_);
  for (int i = 0; i < this->num_probes; i++) {
    if (this->sensors_[i]) {
      if (this->unit_of_measurement_ == this->sensors_[i]->get_unit_of_measurement_ref()) {
        ESP_LOGD(TAG, "Sensor: %s unit set in YAML: %s match device-reported unit: %s",
                 this->sensors_[i]->get_name().c_str(), this->sensors_[i]->get_unit_of_measurement_ref().c_str(),
                 this->unit_of_measurement_);
      } else {
        ESP_LOGW(TAG,
                 "Sensor: %s unit set in YAML: %s does not match device-reported unit: %s. Please update your YAML to "
                 "set the correct unit_of_measurement for this sensor or change the device unit with the iGrill app.",
                 this->sensors_[i]->get_name().c_str(), this->sensors_[i]->get_unit_of_measurement_ref().c_str(),
                 this->unit_of_measurement_);
      }
    }
...
void IGrill::send_authentication_challenge_() {
  ...
  auto status =
      esp_ble_gattc_write_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), app_challenge_handle_,
                               sizeof(zero_payload), zero_payload, ESP_GATT_WRITE_TYPE_RSP, ESP_GATT_AUTH_REQ_NONE);
  if (status) {
...
void IGrill::loopback_device_challenge_response_(uint8_t *raw_value, uint16_t value_len) {
  ...
  auto status =
      esp_ble_gattc_write_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), device_response_handle_,
                               value_len, raw_value, ESP_GATT_WRITE_TYPE_RSP, ESP_GATT_AUTH_REQ_NONE);
  if (status) {
...
void IGrill::request_device_challenge_read_() {
  ESP_LOGV(TAG, "Requesting read of encrypted device response from device on handle (0x%x)",
           this->device_challenge_handle_);
  auto status = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(),
                                        this->device_challenge_handle_, ESP_GATT_AUTH_REQ_NONE);
  if (status) {
    ESP_LOGW(TAG, "Error sending read request for device_challenge, status=%d, handle=0x%x", status,
             this->device_challenge_handle_);
  }
}
...
void IGrill::request_temp_unit_read_() {
  ...
  auto status = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(),
                                        this->temperature_unit_handle_, ESP_GATT_AUTH_REQ_NONE);
  if (status) {
    ESP_LOGW(TAG, "Error sending read request for temperature unit, status=%d, handle=0x%x", status,
             this->temperature_unit_handle_);
  }
}
...
void IGrill::request_read_values_() {
  ...
  if (this->battery_level_sensor_ != nullptr) {
    ...
    status = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(),
                                     this->battery_level_handle_, ESP_GATT_AUTH_REQ_NONE);
    if (status) {
      ESP_LOGW(TAG, "Error sending read request for sensor, status=%d, handle=0x%x", status,
               this->battery_level_handle_);
    }
  }
  ...
  for (auto &probe_handle : probe_handles_) {
    ...
    status = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), probe_handle,
  ...
  if (this->propane_level_sensor_ != nullptr) {
    ...
    status = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(),
                                     this->propane_level_handle_, ESP_GATT_AUTH_REQ_NONE);
    if (status) {
      ESP_LOGW(TAG, "Error sending read request for sensor, status=%d, handle=0x%x", status,
               this->propane_level_handle_);
    }
  }
  ...
  if (pulse_heating_actual1_ || pulse_heating_actual2_ || pulse_heating_setpoint1_ || pulse_heating_setpoint2_) {
    ...
    status = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(),
                                     this->pulse_element_handle_, ESP_GATT_AUTH_REQ_NONE);
    if (status) {
      ESP_LOGW(TAG, "Error sending read request for sensor, status=%d, handle=0x%x", status,
               this->pulse_element_handle_);
    }
  }
}
...
bool IGrill::is_same_address_(uint8_t *a, uint8_t *b) {
  for (size_t i = 0; i < 6; i++) {
    if (a[i] != b[i]) {
      ESP_LOGV(TAG, "Addresses do not match a: %02X:%02X:%02X:%02X:%02X:%02X b: %02X:%02X:%02X:%02X:%02X:%02X", a[0],
               a[1], a[2], a[3], a[4], a[5], b[0], b[1], b[2], b[3], b[4], b[5]);
      return false;
    }
  }
  return true;
...
IGrill::IGrill() : PollingComponent(10000) {}
